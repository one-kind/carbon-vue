<template>
  <header :class="classes" :aria-label="ariaLabel" :aria-labelledby="ariaLabelledby" data-header>
    <slot />
...
import { CarbonPrefix, Aria } from '../../mixins'
...
export default {
  ...
  created() {
    ...
    this.$on('panel-control-mounted', this.onPanelControlMounted);
    this.$on('panel-control-beforeDestroy', this.onPanelControlBeforeDestroy);
    this.$on('panel-control-toggle', this.onPanelControlToggle);
    this.$on('panel-control-focusout', this.onPanelControlFocusout);
    this.$on('panel-mounted', this.onPanelMounted);
    this.$on('panel-beforeDestroy', this.onPanelBeforeDestroy);
    this.$on('panel-focusout', this.onPanelFocusout);
  },
  mounted() {
  ...
  methods: {
    ...
    onPanelControlFocusout(srcComponent, srcEvent) {
    ...
    onPanelFocusout(srcComponent, srcEvent) {
      ...
      if (
        ...
        this.onPanelControlToggle(found, false);
      }
    },
  }
}
