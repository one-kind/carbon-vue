<template>
  <div :class="`${carbonPrefix}--form-item`">
    <label :for="uid" :class="labelClasses">
      {{ !skeleton ? label : '' }}
    </label>
    <div :class="containerClasses">
      <span :class="`${carbonPrefix}--slider__range-label`">{{ !skeleton ? internalMinLabel : '' }}</span>
      <div
        :class="sliderClasses"
        data-slider
      ...
      <span :class="`${carbonPrefix}--slider__range-label`">{{ !skeleton ? internalMaxLabel : '' }}</span>
      <input
        type="number"
        :class="[
...
<script>
import { CarbonPrefix, Uid, Theme, Methods } from '../../mixins'

const name = 'Slider'
export default {
  name,
  mixins: [Uid, Theme, CarbonPrefix, Methods({ thumb: ['blur', 'focus'] })],
  props: {
    disabled: Boolean,
    label: String,
      ...
      validator(val) {
        if (val.length) {
          ...
          if (isNaN(intMultiplier) || intMultiplier < 1) {
            console.warn('Slider: multiplier must be >= 1');
            return false;
          }
        }
        return true;
  ...
  computed: {
    labelClasses () {
      ...
      return {
        [`${carbonPrefix}--label`]: true,
        [`${carbonPrefix}--label--disabled`]: !skeleton && disabled,
    ...
    containerClasses () {
      ...
      return {
        [`${carbonPrefix}--slider-container`]: true,
        [`${carbonPrefix}--skeleton`]: skeleton,
      }
    },
    sliderClasses () {
      ...
      return {
        [`${carbonPrefix}--slider`]: true,
        [`${carbonPrefix}--slider--disabled`]: !skeleton && disabled
      }
    },
    internalMinLabel() {
